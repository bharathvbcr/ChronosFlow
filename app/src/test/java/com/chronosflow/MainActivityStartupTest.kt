...
class MainActivityStartupTest {
    ...
    fun `full shell waits for the first focus window to settle`() {
        val delayMillis = Class.forName("com.chronosflow.MainActivityKt")
    ...
    fun `application startup work waits until the first shell can draw`() {
        val shellDelayMillis = Class.forName("com.chronosflow.MainActivityKt")
            .getDeclaredField("STARTUP_SHELL_DEFER_MILLIS")
            .getLong(null)
        val badgeDataDelayMillis = Class.forName("com.chronosflow.MainActivityKt")
            .getDeclaredField("SHELL_BADGE_DATA_DEFER_MILLIS")
            .getLong(null)
        val notificationChannelDelayMillis = Class.forName("com.chronosflow.ChronosApplicationKt")
            .getDeclaredField("NOTIFICATION_CHANNEL_SETUP_DEFER_MILLIS")
            .getLong(null)
        val applicationDelayMillis = Class.forName("com.chronosflow.ChronosApplicationKt")
    ...
    fun `notification channels are not created before first shell draw`() {
        val source = listOf(
            File("src/main/java/com/chronosflow/ChronosApplication.kt"),
            File("app/src/main/java/com/chronosflow/ChronosApplication.kt")
    ...
    fun `command palette providers wait until palette interaction`() {
        val source = listOf(
            File("src/main/java/com/chronosflow/MainActivity.kt"),
            File("app/src/main/java/com/chronosflow/MainActivity.kt")
        ).first(File::exists).readText()
    ...
    fun `daydial sidebar launch does not eagerly request the data view model`() {
        val source = listOf(
            File("../feature/daydial/src/main/java/com/chronosflow/feature/daydial/DayDialScreen.kt"),
            File("feature/daydial/src/main/java/com/chronosflow/feature/daydial/DayDialScreen.kt")
        ).first(File::exists).readText()
    ...
    fun `portable backup startup is provider backed to avoid eager database open`() {
        val initializerField = Class.forName("com.chronosflow.ChronosApplication")
