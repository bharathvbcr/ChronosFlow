...
class ChronosAIPlannerTest {
    ...
    fun generateIdealDayPlan_fallsBackToLocalHeuristicsWhenNanoFailsInOnDeviceMode() = runTest {
        ...
        every { context.packageName } returns "com.ChronosFlow.VBCR.test"
    ...
    fun generateIdealDayPlan_rescuesMalformedJsonWithOneCorrectiveRetry() = runTest {
        ...
        """.trimIndent()
        // First call uses prefix caching; repair retry falls back to the plain combined prompt.
        coEvery { coordinator.generateAssistTextWithPrefix(any(), any(), any(), any()) } returns
            AssistTextGeneration(text = "sorry, here is your plan (no JSON)", source = AssistGenAiSource.GEMINI_NANO)
        coEvery { coordinator.generateAssistText(any(), any(), any()) } returns
        ...
        coVerify(exactly = 1) { coordinator.generateAssistTextWithPrefix(any(), any(), any(), any()) }
        coVerify(exactly = 1) { coordinator.generateAssistText(any(), any(), any()) }
    }
