...
interface TaskDao {
    ...
    @Query("SELECT id FROM tasks WHERE origin = :origin")
    suspend fun getImportedTaskIds(origin: String): List<String>

    @Query("DELETE FROM tasks WHERE origin = :origin")
    suspend fun deleteAllImportedTasks(origin: String)

    @Query("DELETE FROM tasks WHERE origin = :origin AND id NOT IN (:ids)")
    suspend fun deleteImportedTasksNotIn(origin: String, ids: List<String>)
